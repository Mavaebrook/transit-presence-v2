 ...
 * GtfsRtClient
 *
 * Fetches live GTFS-Realtime feeds and exposes them as StateFlows.
 *
 * IMPORTANT: We do NOT use the gtfs-realtime-bindings library because:
 *   1. It's essentially abandoned (last release 0.0.4 in 2019)
 *   2. It bundles protobuf-java which conflicts with protobuf-javalite
 *   3. The binary format is simple enough to parse with a lightweight
 *      hand-rolled parser for the fields we actually need
 *
 * We parse the raw protobuf bytes manually using the wire protocol:
 *   - Field numbers from the GTFS-RT .proto spec
 *   - Only the fields we need: vehicle position, trip update, arrival times
 */
@Singleton
class GtfsRtClient @Inject constructor(
    ...
    /**
     * Compute predicted arrivals for a stop from current trip updates.
     */
    fun arrivalsForStop(
        stopId: String,
        routeId: String?,
...
// Parses only the fields we need from the GTFS-RT binary format.
// Field numbers from: https://github.com/google/transit/blob/master/gtfs-realtime/proto/gtfs-realtime.proto
...
class ProtoReader(private val bytes: ByteArray) {
    ...
    fun skipField() {
        // Read the tag we already consumed to determine wire type
        // Since we called readTag() first, peek at wire type from last tag
        // For safety, skip a varint (most common case)
        readVarInt()
    }
}
