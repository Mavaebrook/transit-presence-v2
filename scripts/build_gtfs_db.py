...
Updated with aggressive whitespace stripping and improved indexing.
"""
...
def build_db(gtfs_bytes):
    ...
    with zipfile.ZipFile(io.BytesIO(gtfs_bytes)) as zf:
        ...
        cursor.executemany("INSERT OR REPLACE INTO stops VALUES (?, ?, ?, ?, ?)", 
            ((clean(r["stop_id"]), clean(r.get("stop_name", "")), float(r["stop_lat"]), float(r["stop_lon"]), int(r.get("wheelchair_boarding") or 0))
             for r in iter_csv_from_zip(zf, "stops.txt")))

        print("Parsing routes...")
        cursor.executemany("INSERT OR REPLACE INTO routes VALUES (?, ?, ?, ?, ?, ?)",
            ((clean(r["route_id"]), clean(r.get("route_short_name", "")), clean(r.get("route_long_name", "")), int(r.get("route_type") or 3), clean(r.get("route_color", "FFFFFF")), clean(r.get("route_text_color", "000000")))
             for r in iter_csv_from_zip(zf, "routes.txt")))

        print("Parsing trips...")
        cursor.executemany("INSERT OR REPLACE INTO trips VALUES (?, ?, ?, ?, ?, ?)",
            ((clean(r["trip_id"]), clean(r["route_id"]), clean(r["service_id"]), clean(r.get("trip_headsign", "")), int(r.get("direction_id") or 0), clean(r.get("shape_id", "")))
             for r in iter_csv_from_zip(zf, "trips.txt")))

        print("Parsing stop_times (cleaning and normalizing)...")
        cursor.executemany("INSERT OR REPLACE INTO stop_times VALUES (?, ?, ?, ?, ?)",
            ((clean(r["trip_id"]), clean(r["stop_id"]), int(r["stop_sequence"]), normalize_time(r["arrival_time"]), normalize_time(r["departure_time"]))
        ...
        cursor.executemany("INSERT OR REPLACE INTO calendar VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", 
            ((clean(r["service_id"]), int(r["monday"]), int(r["tuesday"]), int(r["wednesday"]), int(r["thursday"]),
              int(r["friday"]), int(r["saturday"]), int(r["sunday"]), clean(r["start_date"]), clean(r["end_date"]))
             for r in iter_csv_from_zip(zf, "calendar.txt")))

        print("Parsing shapes...")
        cursor.executemany("INSERT OR REPLACE INTO shapes VALUES (?, ?, ?, ?)",
            ((clean(r["shape_id"]), float(r["shape_pt_lat"]), float(r["shape_pt_lon"]), int(r["shape_pt_sequence"]))
